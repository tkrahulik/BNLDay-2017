...
Superconductors, when cooled below their critical temperature ($T_{c}$), expel magnetic fields through the Meissner effect. We have obtained a roll of Yttrium Barium Copper Oxide (YBCO) tape from American Superconductor, which consists of a thin layer of YBCO attached to a Nickel backing and coated with a thin layer of Silver to prevent corrosion. Here, we show that multiple layers of this tape, shaped into cylinders, can be used as an effective means to shield the inside of the structure from an external field. We also show that the shielding of this system can be increased through the addition of more layers.
...
Superconductors alone can create a field-free region in the interior of the cylinder, but distort the external field (Figure \ref{fig:distortions}). For particle accelerators, where a uniform field is necessary for momentum measurements of charged particle, this is not an ideal situation. By adding a carefully tuned ferromagnet concentric with the superconductor, we have shown that we can reduce the distortions by $90\%$ (Figure \ref{fig:cloak}).
